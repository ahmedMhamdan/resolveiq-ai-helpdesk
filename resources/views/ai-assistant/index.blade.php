    ...
    <div class="page-head ai-page-head">
        <div>
            ...
            <p class="page-subtitle">
                Generate ticket summaries, suggested replies, priority recommendations, due date suggestions, or a custom AI response.
            </p>
        </div>
    </div>
            ...
            >
                ...
                <div class="ai-generate-row">
                    <button type="submit" class="btn btn-primary ai-generate-btn" id="generateAiBtn">
                        ...
                        <span class="ai-btn-text">Generate Response</span>
                ...
                <div id="aiOutputContent">
                    @if ($ticketAi)
                        ...
                        @if (! empty($ticketAi['used_fallback']))
                            <div class="ai-fallback-notice">
                                This response was generated by the local mock fallback. Check your AI API key or provider settings.
                            </div>
                        @endif
                        ...
                        <div class="ai-generated-text">
                            @foreach ($aiLines as $line)
                                @php
                                    $cleanLine = trim($line);
                                    $isStrongLine = preg_match('/^([0-9]+\.\s|Priority:|Reason:|Main issue:|Important context:|Current status:|Recommended next step:|Due Date:|Suggested Due Date:|تاريخ الاستحقاق:|السبب:)/i', $cleanLine);
                                @endphp
                    ...
                    @else
                        <div class="ai-empty-state">
                            ...
                            <span>
                                Select a ticket, choose an action, then generate a response.
                            </span>
                        </div>
                    @endif
                </div>
    ...
    <script>
        (() => {
            ...
            function isStrongAiLine(line) {
                return /^(\d+\.\s|Priority:|Reason:|Main issue:|Important context:|Current status:|Recommended next step:|Created:|Assignment:|Customer\/support conversation:|Status and priority changes:|Current state:|Recommended follow-up:|Due Date:|Suggested Due Date:|تاريخ الاستحقاق:|السبب:|المشكلة الرئيسية:|السياق المهم:|الحالة الحالية:|الخطوة المقترحة:|تاريخ الإنشاء:|التعيين:|المحادثة:|تغييرات الحالة والأولوية:|الوضع الحالي:|المتابعة المقترحة:)/i.test(line);
            }
            ...
            function renderAiOutput(ticketAi) {
                ...
                const fallbackNotice = ticketAi.used_fallback
                    ? '<div class="ai-fallback-notice">This response was generated by the local mock fallback. Check your AI API key or provider settings.</div>'
                    : '';
            ...
            function setLoading(isLoading) {
                ...
                if (btnText) {
                    btnText.textContent = isLoading ? 'Generating' : 'Generate Response';
                }
            }
            ...
            generateForm?.addEventListener('submit', async event => {
                ...
                try {
                    ...
                    if (!response.ok) {
                        ...
                        throw new Error(errors || data.message || 'Could not generate AI response.');
                    }

                    renderAiOutput(data.ticket_ai);
                } catch (error) {
                    alert(error.message || 'Could not generate AI response.');
